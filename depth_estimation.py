...
def parse_element(example):
    ...
    features = tf.compat.v1.parse_single_example(example,
                                       features={
                                           'RGB': tf.compat.v1.FixedLenFeature([], tf.string),
                                           'DPPhi': tf.compat.v1.FixedLenFeature([], tf.string),
                                           'DP': tf.compat.v1.FixedLenFeature([], tf.string),
                                       })

    RGB_flat = tf.compat.v1.decode_raw(features['RGB'], tf.uint8)
    RGB = tf.reshape(RGB_flat, [N, N, 3])

    DPPhi_flat = tf.compat.v1.decode_raw(features['DPPhi'], tf.uint8)
    DPPhi = tf.reshape(DPPhi_flat, [N, N, N_Phi])

    DP_flat = tf.compat.v1.decode_raw(features['DP'], tf.uint8)
...
def data_augment(RGB_batch_float, DPPhi_float, Phi_batch_scaled):
    ...
    r1 = tf.compat.v1.random_uniform([]) * 0.3 + 0.8
...
def read2batch(TFRECORD_PATH, batchsize):
    ...
    RGB_batch, DPPhi_batch, DP_batch = next(iterator)
...
def fft2dshift(input):
    dim = int(input.shape[1])  # dimension of the data
    if dim % 2 == 0:
        print('Please make the size of kernel odd')
    channel1 = int(input.shape[0])  # channels for the first dimension
    # shift up and down
...
def gen_PSFs(h, OOFphase, wvls, idx, N_R, N_G, N_B):
    ...
    with tf.compat.v1.variable_scope("Red"):
        ...
        PSF_R = tf.divide(tf.square(tf.abs(fft2dshift(tf.compat.v1.fft2d(Pupil_R)))), Norm_R, name='PSF_R')

    with tf.compat.v1.variable_scope("Green"):
        ...
        PSF_G = tf.divide(tf.square(tf.abs(fft2dshift(tf.compat.v1.fft2d(Pupil_G)))), Norm_G, name='PSF_G')

    with tf.compat.v1.variable_scope("Blue"):
        ...
        PSF_B = tf.divide(tf.square(tf.abs(fft2dshift(tf.compat.v1.fft2d(Pupil_B)))), Norm_B, name='PSF_B')
...
with tf.compat.v1.variable_scope("PSFs"):
    a_zernike = tf.compat.v1.get_variable("a_zernike", [N_modes, 1], initializer=tf.zeros_initializer(),
